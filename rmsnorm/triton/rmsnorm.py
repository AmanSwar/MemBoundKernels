...
class RMSNormTriton(torch.nn.Module):
    ...
    def __init__(self , embed_dim , eps=1e-6 , weight : torch.Tensor = None):

        super().__init__()

        self.weight = torch.nn.Parameter(torch.ones(embed_dim)) if weight is None else torch.nn.Parameter(weight)
        self.eps = eps
...
if __name__ == "__main__":
    ...
    def benchmark_rmsnorm_feature_dim(M, N, provider):
        ...
        # Calculate GFLOPS
        total_flops = 3 * M * N  
        gflops = total_flops / (ms * 1e-3) / 1e9
    ...
    benchmark_rmsnorm_plot.run(show_plots=True , save_path="./")
    print("Running RMSNorm benchmark (varying feature dimension)...")
    benchmark_rmsnorm_feature_dim.run(show_plots=True , save_path="./")
