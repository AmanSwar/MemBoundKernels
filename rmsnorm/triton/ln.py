...
#         styles=[('blue', '-'), ('green', '-')],
#         ylabel='GB/s',
#         plot_name='layer-norm-forward',  # Changed from backward
#         args={'M': 4096, 'dtype': torch.float16, 'mode': 'forward'},  # Changed to forward
#     ))
...
def bench_layer_norm(M, N, dtype, provider, mode='forward', eps=1e-5, device=DEVICE):
    ...
    # forward pass
    if mode == 'forward':
        gflops = lambda ms: 2 * x.numel() * 1e-9 / (ms * 1e-3)
        ms, min_ms, max_ms = triton.testing.do_bench(y_fwd, quantiles=quantiles, rep=500)
